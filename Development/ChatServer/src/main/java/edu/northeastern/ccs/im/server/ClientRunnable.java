...
public class ClientRunnable implements Runnable {
	...
	private void checkForInitialization() {
		...
		if (messageIter.hasNext()) {
			...
			ChatLogger.info("handling"+msg.getMsgType());
			IncomingMessageHandler.choose(msg).run(this);
		}
	}
	 ...
	 */
	boolean messageChecks(Message msg) {
	 ...
	 * 
	 * @param message messages to be sent immediately.
	 ...
	 */
	public void setUserId(String uniqueId)
	{
	...
	private boolean setUserName(String userName) {
		...
		if (userName != null) {
			...
			setName(userName);
			userId = Integer.toString(hashCode());
			result = true;
		} else {
	...
	protected void handleIncomingMessages() {
		...
		if (messageIter.hasNext()) {
			...
			// Determine the correct message handling strategy and run it
			ChatLogger.info("inside handling incoming messages "+ msg.messageType());
			IncomingMessageHandler.choose(msg).run(this);
		}
	}
	...
	public void terminateClient() {
		...
		Prattle.getInstance().removeClient(this);
